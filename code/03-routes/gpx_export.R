...
top30_stops <- st_read(
  here("data/processed/top30_streets.geojson"),
  quiet = TRUE
)
...
osrm_route <- function(stops_sf, profile = "foot") {
  ...
  track_coords <- do.call(
    rbind,
    lapply(coords_list, function(c) c(c[[1]], c[[2]]))
  )
...
# --- Top 10: Run (foot profile) ---
cat("Fetching top-10 run route from OSRM...\n")
run10_route <- osrm_route(walk_ordered, profile = "foot")

if (!is.null(run10_route)) {
  cat(sprintf(
    "Top-10 run: %.1f km (road-following)\n",
    run10_route$distance_m / 1000
  ))
}

write_gpx(
  stops_sf = walk_ordered,
  track_coords = run10_route$track,
  name = "Vancouver Cherry Blossom Marathon — Run (Top 10)",
  description = "Top 10 cherry blossom streets by density. Foot-routing via OSRM.",
  path = here("data/routes/cherry-blossom-top10-run.gpx")
)

# --- Top 10: Cycling (bike profile) ---
cat("Fetching top-10 cycling route from OSRM...\n")
cycle10_route <- osrm_route(walk_ordered, profile = "bike")

if (!is.null(cycle10_route)) {
  cat(sprintf(
    "Top-10 cycling: %.1f km (road-following)\n",
    cycle10_route$distance_m / 1000
  ))
}

write_gpx(
  stops_sf = walk_ordered,
  track_coords = cycle10_route$track,
  name = "Vancouver Cherry Blossom Marathon — Cycling (Top 10)",
  description = "Top 10 cherry blossom streets by density. Bike-routing via OSRM.",
  path = here("data/routes/cherry-blossom-top10-cycling.gpx")
)
...
if (!is.null(bike_route)) {
  cat(sprintf(
    "Bike route: %.1f km (road-following)\n",
...
# --- Run route (all 30, foot profile) ---
# foot profile allows pedestrian paths, park shortcuts, sidewalks
# — more accurate than bike profile for running
cat("Fetching run route from OSRM...\n")
run_route <- osrm_route(bike_ordered, profile = "foot")

if (!is.null(run_route)) {
  cat(sprintf(
    "Run route: %.1f km (road-following)\n",
    run_route$distance_m / 1000
  ))
}

write_gpx(
  stops_sf = bike_ordered,
  track_coords = run_route$track,
  name = "Vancouver Cherry Blossom Grand Tour — Run",
  description = "Top 30 cherry blossom streets in Vancouver by density. Foot-routing via OSRM (sidewalks, paths, park shortcuts).",
  path = here("data/routes/cherry-blossom-run.gpx")
)

cat("\nDone. GPX files saved to data/routes/\n")
cat(
  "Import into: Strava, Garmin Connect, Komoot, AllTrails, Google Maps (via My Maps)\n"
)
